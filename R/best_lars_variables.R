best_lars_variables <- function(y_xx, y_name, cut_date = NULL, use_if_corr = NULL) {
  ...
  xx     <- y_xx[, !grepl(y_name, names(y_xx)), drop = FALSE]
  
  # Filter y and yy to include only observations on or after cut_date
  y_cut  <- y[xx$date >= cut_date, , drop = FALSE]
  xx_cut <- xx[xx$date >= cut_date, ]
  ...
  if (length(xx_cut) == 0) {
    stop("Balanced xx contains no data. Consider using later cut date. (see line 45 in 'best_lars_variables.R')")
  }
  ...
  # run 
  predictor_names <- setdiff(names(combined_data), c("date", y_name))
  y   <- as.matrix(combined_data[[y_name]])
